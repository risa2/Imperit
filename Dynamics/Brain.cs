...
namespace Imperit.Dynamics
{
    public class Brain
    {
        ...
        List<ICommand> DefensiveRecruitments(State.Settings settings, State.Provinces provinces, PInfo[] info, int[] my)
        {
        ...
        List<ICommand> StabilisatingRecruitments(State.Settings settings, State.Provinces provinces, PInfo[] info, int[] my)
        {
            var result = new List<ICommand>();
            foreach (int i in my)
            {
        ...
        IEnumerable<ICommand> Recruitments(State.Settings settings, State.Provinces provinces, PInfo[] info, int[] my)
        {
            var result1 = DefensiveRecruitments(settings, provinces, info, my);
            var result2 = StabilisatingRecruitments(settings, provinces, info, my);
            if (player.Money > 0)
            {
        ...
        void Transport(List<ICommand> result, State.Settings settings, State.Provinces provinces, PInfo[] info, int from, int to, uint count)
        {
        ...
        List<ICommand> SpreadSoldiers(State.Settings settings, State.Provinces provinces, PInfo[] info, int[] my)
        {
            ...
            foreach (int from in my)
            {
                foreach (var dest in provinces.NeighborsOf(provinces[from]).Where(n => n.IsControlledBy(player)).OrderBy(n => info[n.Id].Bilance))
                {
                    ...
                    {
                        Transport(result, settings, provinces, info, from, dest.Id, info[from].Soldiers);
                    }
                    else if (info[from].Enemies <= 0 && info[dest.Id].Enemies <= 0)
                    {
                        Transport(result, settings, provinces, info, from, dest.Id, info[from].Soldiers / (uint)Max(provinces.NeighborCount(provinces[from]) - 1, 1));
                    }
                    else if (info[from].Bilance > 0 && info[from].Bilance >= info[dest.Id].Bilance)
                    {
                        Transport(result, settings, provinces, info, from, dest.Id, (uint)Min(info[from].Bilance, info[from].Soldiers));
                    }
                }
            }
            return result;
        ...
        public IEnumerable<ICommand> Think(State.Settings settings, State.Provinces provinces)
        {
            ...
            var result = Recruitments(settings, provinces, info, my);
