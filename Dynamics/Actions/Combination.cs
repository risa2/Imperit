...
namespace Imperit.Dynamics.Actions
{
    public class Combination : IAction
    {
        ...
        public IAction Do(IArray<State.Player> players, State.Provinces provinces, int active) => new Combination(Actions.Select(action => action.Do(players, provinces, active)).Where(action => !(action is Nothing)));
    }
}